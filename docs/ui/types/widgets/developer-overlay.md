...
This is the type each plugin's `UN<Plugin>DeveloperOverlay` derives from — for example, [`UNActorPoolsDeveloperOverlay`](/docs/actor-pools/developer-overlay.md), [`UNDynamicRefsDeveloperOverlay`](/docs/dynamic-references/developer-overlay.md), and [`UNGuardianDeveloperOverlay`](/docs/guardian/developer-overlay.md). Build your own diagnostic surface by subclassing this widget and adding rows to `ContainerBox`.
