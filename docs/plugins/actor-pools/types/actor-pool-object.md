...
- **Surfaces Pool Configuration**: `DoesSupportInterface()`, `ShouldInvokeUFunctions()`, and `GetDescription()` expose the pool's flags and a human-readable description, used by the overlay's tooltips and color swatches.
