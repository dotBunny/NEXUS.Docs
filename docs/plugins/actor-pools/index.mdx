...
Pooling works out of the box for any `AActor` whose **root is a `UPrimitiveComponent`** — the pool hides/shows it, toggles collision and tick, and resets velocity and physics simulation on spawn/return without any code on the Actor. The [INActorPoolItem](/docs/plugins/actor-pools/types/actor-pool-item/) interface (and [ANPooledActorBase](/docs/plugins/actor-pools/types/pooled-actor-base/) class) are **native-only** and only needed when an Actor requires extra lifecycle logic the pool can't infer.
...
The real advantage of pooling objects comes from when you have an opportunity to create an approximate number of objects you are going to need during a non-performance-sensitive moment (think loading screen). There are a few options to being able to do this with [FNActorPools](/docs/plugins/actor-pools/types/actor-pool/).
...
![NActorPools](/assets/images/docs/plugins/actor-pools/actor-pools-samples.webp)
