...
A graph also records the `FGuid` of the organ component that built it. That is what lets a later stage resolve back to the authoring [Organ Component](../types/organ-component.md) — which the [connector pass](tasks.md#junction-connecting) needs in order to honour organ-level overrides for the cells a graph placed. A graph built outside an organ (tests, ad-hoc construction) carries an invalid identifier.
