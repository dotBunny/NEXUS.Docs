...
When a thumbnail is captured for the `ANCellActor` in the level editor (via the **Capture Thumbnail** button in [Editor Mode](../editor-mode/cell-editor.md)), it propagates to the side-car automatically so the cell shows the same preview in the content browser as the source level.
