...
An [Assembly Operation](../types/assembly-operation.md) is a coordinator, not a worker. Once its context is locked it hands the actual work to a task graph, which moves through **four phases**.
...
**3 · Join** — closes the gaps the builders could not.

```mermaid
flowchart LR
    F[Gather Open Junctions] --> G[Mate Coincidences] --> H[Route &amp; Accept]
```

Runs once, after every pass has collected. See [Junction Connecting](tasks.md#junction-connecting).

**4 · Place** — the only phase that touches the real world again.

```mermaid
flowchart LR
    I[Create Spawns] --> J[Spawn Cell Proxies] --> K[Finalize]
```

Accepted junction pairings are handed to the subsystem alongside the proxy spawning, not after it.
...
**The game thread is a scarce resource.** The graph builder runs off-thread, which is why the pipeline snapshots what it needs into contexts up front rather than reaching into the [Registry](../types/world-assembly-registry.md) — that registry is game-thread only. Only four of the nine [task-graph jobs](tasks.md#the-jobs) are pinned to the game thread, and [each for a concrete engine reason](tasks.md#why-the-game-thread-jobs-are-pinned).

**Joining is a phase of its own because building cannot do it.** Organ builders only ever grow a *new* cell off an open junction, so two cells whose openings end up facing each other across clear space are invisible to each other during the build. Only once every pass has collected is there a complete layout to match against — which is why Join sits where it does rather than inside Build.
...
**The connector stage is deterministic without consulting a stream at all.** Candidate ordering, corner correspondence, and detour ordering are derived from the geometry, so it reproduces from the layout rather than from the seed — see [Determinism](tasks.md#determinism).
