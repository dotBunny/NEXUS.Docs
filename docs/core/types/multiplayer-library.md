...
If you already have a `UWorld*` pointer, prefer calling [`FNMultiplayerUtils`](https://github.com/dotBunny/NEXUS/blob/main/core/Source/NexusCore/Public/NMultiplayerUtils.h) directly — every method here forwards to one of its `FORCEINLINE` statics. The native utility also exposes a `ServerTravel` wrapper that is not surfaced through Blueprint.
