...
### 1. `docs/<slug>/index.mdx`

```mdx
---
description: <Description from .uplugin>
sidebar_position: <next integer — read existing docs/*/index.mdx files, take max sidebar_position + 1>
---
...
### 3. (Only if the plugin has an Editor module) `docs/<slug>/editor-types/index.mdx`

Same as `types/index.mdx` but with `sidebar_position: 2` and `editor types added by <ShortName>` wording — match phrasing in [docs/core/editor-types/index.mdx](docs/core/editor-types/index.mdx).
...
**Sidebar ordering inside `types/` and `editor-types/`**: subfolders sort first (alphabetical among themselves), then at-level pages. Concretely, if `types/` will have `N` subfolders, their `index.mdx` files take positions `1..N` (alpha), and any at-level `.md` files start at `N+1`. Adding a new subfolder later means bumping every at-level page's `sidebar_position` by one. See [CLAUDE.md](CLAUDE.md) Sidebar ordering for the full rule. For canonical subfolder shapes see [docs/core/types/math/index.mdx](docs/core/types/math/index.mdx) and [docs/ui/types/components/index.mdx](docs/ui/types/components/index.mdx).
...
- [docs/actor-pools/developer-overlay.md](docs/actor-pools/developer-overlay.md)
- [docs/dynmaic-references/developer-overlay.md](docs/dynmaic-references/developer-overlay.md)
- [docs/guardian/developer-overlay.md](docs/guardian/developer-overlay.md)

The abstract base these subclass — `UNDeveloperOverlay` — is itself documented as a normal type page at [docs/ui/types/widgets/developer-overlay.md](docs/ui/types/widgets/developer-overlay.md) (it lives in the UI plugin and subclasses `UCommonUserWidget`). Don't confuse the two: per-plugin overlays go at the plugin root with `base="UNDeveloperOverlay"`; the base widget is a UI type page with `base="UCommonUserWidget"`.

### File: `docs/<slug>/developer-overlay.md`
...
By going to `Tools > NEXUS > <PluginName>`, you can create an [UNEditorUtilityWidget](/docs/ui/editor-types/editor-utility-widget/) wrapped version of `/Nexus<Plugin>/WB_N<Plugin>DeveloperOverlay` which will <TODO: short description of what it shows>.

<div class="image-split">
![No <Things>](/assets/images/docs/<slug>/<slug>-developer-overlay-none.webp)
![<Things>](/assets/images/docs/<slug>/<slug>-developer-overlay.webp)
</div>
...
- The two screenshots are the user's responsibility — flag them as TODOs. They belong at `static/assets/images/docs/<slug>/<slug>-developer-overlay-none.webp` and `…-developer-overlay.webp`, referenced by **absolute** URL as shown above. Never co-locate an image next to the page — see the static-asset conventions in CLAUDE.md.
