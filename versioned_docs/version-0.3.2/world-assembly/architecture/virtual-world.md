...
Those two phases stay in **two separate arrays** rather than being appended into one, and the reason is entirely about who may touch what, and when:
...
**Node collision is parallel to `NodeIndex`**, which is what lets a collision hit resolve back to the cell node it belongs to.
