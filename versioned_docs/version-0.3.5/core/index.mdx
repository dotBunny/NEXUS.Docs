...
- **PCG Elements:** A set of [graph nodes](types/pcg/index.mdx) under the **NEXUS** category for shaping point layouts, plus the marker component one of them reads back out of a level. `NexusCore.uplugin` enables Epic's `PCG` plugin, so they are present in any project with NEXUS installed.
