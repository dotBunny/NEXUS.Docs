...
| ✅ | Drafting unit or performance tests for an existing class. | There is enough context in the project to generate useful tests. As always, they still need to be confirmed correct and complete by a developer. |
| ✅ | Drafting Doxygen comments for an existing header. | Project skills can produce a reasonable first pass of in-source documentation directly from the code. A developer still needs to confirm each comment matches actual behavior. |
| ✅ | Drafting a documentation page (this site) from an existing header. | Type pages here are generated by the [`doc-new-type`](https://github.com/dotBunny/NEXUS.Docs) skill from a real source header. The output is a starting point, not a final draft. |
...
| ❌ | Submitting AI-generated tests without verifying they catch failure. | A test that always passes is worse than no test. |
| ❌ | Letting AI write the commit message or PR description without review. | AI is too prone to hallucinating motivations and missing the actual reason for a change. Reviewers rely on these to be accurate — which means you need to understand the change well enough to write them yourself. |
